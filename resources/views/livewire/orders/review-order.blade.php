                                        ...
                                        <div class="flex flex-col items-start gap-y-6">
                                            <div class="flex justify-start">
                                                ...
                                                <div class="flex flex-col items-start gap-y-1">
                                                    <x-rating-input wire:model.number="reviews.{{$item->id}}.rating" />
                                                    <x-input-error
                                                        :messages="$errors->get('reviews.' . $item->id .'.rating')" />
                                                </div>

                                            </div>


                                            <div x-id="['review']" class="flex justify-start">
                                                <label :for="$id('review')">评论：</label>

                                                <div class="flex flex-col items-start gap-y-1">
                                                    <textarea
                                                        :id="$id('review')"
                                                        wire:model="reviews.{{$item->id}}.review"
                                                        placeholder="分享您的体验心得，输入评论"
                                                        class="resize-none focus:border-black focus:ring-black text-sm w-96 h-32"></textarea>

                                                    <x-input-error :messages="$errors->get('reviews.' . $item->id .'.review')" />
                                                </div>
