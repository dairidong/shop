<div class="container mt-14 relative">
    ...
    <form wire:submit="createOrder">
                    ...
                    <li class="flex justify-around py-6">
                        <div class="w-1/4 flex justify-center items-center">
                            <a href="{{ route('products.show',[$item['sku']->product->id]) }}"
                               wire:navigate>
                                @if($image = $item['sku']->product->getFirstMediaUrl())
                                    <img src="{{ $image }}" />
                                @else
                            ...
                            <div>
                                <div>
                                    <a class="hover:text-active"
                                       href="{{ route('products.show',[$item['sku']->product->id]) }}"
                                       wire:navigate
                                    >
                                        {{ $item['sku']->product->title }}
                                    </a>
                                </div>
                                <div class="text-gray-400">{{ $item['sku']->name }}</div>
                            </div>
                        </div>
        ...
        <section class="flex items-center justify-end gap-6">
            @if(isset($showBackToCart) && $showBackToCart)
